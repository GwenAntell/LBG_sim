rarefy <- function(data = data, bandsize = 15, sample = 50, iterations = 100){
  ...
  for(j in reps){
    ...
    occurrences_per_latbin <- matrixStats::rowMeans2(raredf)
  ...
  latbin$mean_richness <- matrixStats::rowMeans2(df)
  quant <- matrixStats::rowQuantiles(df, probs = c(0.025, 0.975))
  latbin$CI.Lower <- quant[,1]
  latbin$CI.Upper <- quant[,2]
  
  rows <- which(latbin$mean_richness == 0)
  
  plot(mean_richness~mid, latbin,  main = "Latitudinal biodiversity gradient", ylab = "Species richness", xlab = expression(Latitude~(degree)), pch = 20, ylim = c(min(latbin$CI.Lower, na.rm = TRUE), max(latbin$CI.Upper, na.rm = TRUE)))
