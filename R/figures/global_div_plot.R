...
flat_plot <- ggplot() +
  geom_rect(data = throwing_shade, mapping=aes(xmin=min_age, xmax=max_age, ymin=-20, ymax= 320), linetype = 0, color="grey90", alpha=0.1)  +
  geom_segment(data = periods, mapping=aes(x = min_age, xend = min_age, y = -20, yend = 320), linetype = 2, size = 1, color = "grey80") +
