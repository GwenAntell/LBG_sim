...
simulated <- data[,c("simulated_SLD", "mid_age")]
...
p <- ggplot(data = data, aes(colour = type)) +
  ...
  geom_rect(data = periods, mapping=aes(xmin=300, xmax=0, ymin=-0.3, ymax= 0), linetype = 1, colour = "black", fill="black", alpha=1)  +
  geom_rect(data = periods, mapping=aes(xmin=min_age, xmax=max_age, ymin= -0.3, ymax= 00), linetype = 1, colour = "black", fill=periods$color, alpha=1)  +
  geom_text(data = periods, mapping=aes(x=mid_age, y= -0.15, label = abbr), colour = "black", alpha=1)  +
  ...
  scale_y_continuous(expand=c(0,0), limits = c(-0.3, 6.5)) +
  labs(x = "Time (Ma)", y = "Sum of latitudinal richness differences", title = "") +
...
ggsave(file="./figures/SLD_LBG_type.png", plot = p, width = 190, height = 130, units = "mm", dpi = 300, scale = 1.25)
