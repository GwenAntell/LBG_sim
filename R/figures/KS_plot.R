...
# #unimodal and flat
# 
# type <- read.csv("./results/KS_test/LBG_type_temporal_KS_test_UF.csv")
# type$sim_sig <- NA
# type$sim_sig[which(type$sim_ks_sig <= 0.05)] <- col[1]
# 
# type$samp_sig <- NA
# type$samp_sig[which(type$samp_ks_sig <= 0.05)] <- col[2]
# 
# type$rare_sig <- NA
# type$rare_sig[which(type$rare_ks_sig <= 0.05)] <- col[3]
# ##---------------------------------
# 
# type_plot_1 <- ggplot() +
#   geom_rect(data = throwing_shade, mapping=aes(xmin=min_age, xmax=max_age, ymin=-0.075, ymax= 1.1), linetype = 0, color="grey90", alpha=0.1)  +
...
#   geom_rect(data = periods, mapping=aes(xmin=min_age, xmax=max_age, ymin= -0.075, ymax= 0), linetype = 1, colour = "black", fill=periods$color, alpha=1)  +
#   geom_text(data = periods, mapping=aes(x=(min_age+max_age)/2, y= -0.0375, label = abbr), colour = "black", alpha=1)  +
#   geom_line(data = type, aes(x = mid_age, y = sim_ks_stat), colour = col[1], size = 1.2) +
#   geom_point(data = type, aes(x = mid_age, y = sim_ks_stat), shape = 21, colour= "black", fill = type$sim_sig, size = 2.5) +
#   #geom_line(data = type, aes(x = mid_age, y = samp_ks_stat), colour = col[2], size = 1.2) +
#   #geom_point(data = type, aes(x = mid_age, y = samp_ks_stat), shape = 21, colour= "black", fill = type$samp_sig, size = 2.5) +
#   #geom_line(data = type, aes(x = mid_age, y = rare_ks_stat), colour = col[3], size = 1.2) +
#   #geom_point(data = type, aes(x = mid_age, y = rare_ks_stat), shape = 21, colour= "black", fill = type$rare_sig, size = 2.5) +
#   scale_x_reverse(expand=c(0,0), breaks = seq(0, 300, 50), labels = seq(0, 300, 50)) +
#   scale_y_continuous(expand = c(0,0)) +
#   labs(x = "", y = expression(bold(paste("Kolmogorov-Smirnov score (", italic("D"), ")"))), title = "Simulated", subtitle = "Unimodal- and flat-type") +
#   theme(panel.background = element_blank(),
#         plot.margin = margin(0.5,0.5,0.5,0.5, "cm"),
#         panel.border = element_rect(colour = "black", fill = NA),
#         axis.text.x = element_text(size = 14, angle = 0, hjust = 0.5),
...
# 
# type <- read.csv("./results/KS_test/LBG_type_temporal_KS_test_UB.csv")
# type$sim_sig <- NA
# type$sim_sig[which(type$sim_ks_sig <= 0.05)] <- col[1]
# 
# type$samp_sig <- NA
...
#   scale_x_reverse(expand=c(0,0), breaks = seq(0, 300, 50), labels = seq(0, 300, 50)) +
#   scale_y_continuous(expand = c(0,0)) +
#   labs(x = "", y = " Kolmogorov-Smirnov statistic ", title = "", subtitle = "Unimodal- and bimodal-type") +
#   theme(panel.background = element_blank(),
#         plot.margin = margin(0.5,0.5,0.5,0.5, "cm"),
#         panel.border = element_rect(colour = "black", fill = NA),
#         axis.text.x = element_text(size = 14, angle = 0, hjust = 0.5),
...
# #bimodal and flat
# 
# type <- read.csv("./results/KS_test/LBG_type_temporal_KS_test_FB.csv")
# type$sim_sig <- NA
# type$sim_sig[which(type$sim_ks_sig <= 0.05)] <- col[1]
# 
# type$samp_sig <- NA
...
#   scale_x_reverse(expand=c(0,0), breaks = seq(0, 300, 50), labels = seq(0, 300, 50)) +
#   scale_y_continuous(expand = c(0,0)) +
#   labs(x = "Time (Ma)", y = " Kolmogorov-Smirnov statistic ", title = "", subtitle = "Flat- and bimodal-type") +
#   theme(panel.background = element_blank(),
#         plot.margin = margin(0.5,0.5,0.5,0.5, "cm"),
#         panel.border = element_rect(colour = "black", fill = NA),
#         axis.text.x = element_text(size = 14, angle = 0, hjust = 0.5),
...
# 
# p
# ggsave("./figures/ks_type_plot.png", plot = p, width = 300, height = 220, units = "mm", dpi = 600, scale = 1.6)
# 
