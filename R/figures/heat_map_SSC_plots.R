...
ssc_plot <- ggplot() + 
  ...
  scale_fill_viridis(option = "D", direction = 1, limits=c(0, 13), breaks=seq(0,13,by=1), labels = seq(0,13,by=1)) +
  geom_tile(data = stages, aes(x = mid_age, y = 95, width = duration, height = 10), colour = "black", fill= stages$periodcol)+
  geom_text(data = periods, aes(x = mid_age, y = 95, label = abbr), color = "black", size = 3) +
  ...
  theme(
    ...
    axis.title.x=element_text(size = 10, face = "bold", vjust = -4, colour = "Black"),
    axis.title.y=element_text(size = 10, face = "bold", vjust = 4, colour = "black"),
    ...
    legend.title = element_text(size = 10, face = "bold", vjust = -6, colour = "white"),
...
ssc_plot <- ssc_plot + guides(fill = guide_colourbar(ticks.colour = "black", frame.colour = "black", label = TRUE, title = "Spatial Sampling Coverage (%)", title.position = "top", title.hjust = 0.5, title.vjust = -9))

ssc_plot

png("./figures/heat_map_ssc.png",width = 210, height = 140, units = "mm", res = 600)
ssc_plot
dev.off()
